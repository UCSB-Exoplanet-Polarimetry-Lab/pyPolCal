...
def get_existing_progress(h5_path):
    ...
    with h5py.File(h5_path, "r") as h5_file:
        ...
        attrs = mcmc_group.attrs
        saved_steps = int(attrs.get("iteration", 0))
        allocated_steps = int(mcmc_group["chain"].shape[0])
        if allocated_steps > saved_steps:
            print(
                f"Warning: {h5_path} has storage allocated for {allocated_steps} "
                f"samples, but emcee reports only {saved_steps} written samples. "
                "Only the written samples are usable."
            )
...
def record_run_metadata(h5_path):
    ...
    with h5py.File(h5_path, "r+") as h5_file:
        ...
        saved_steps = int(attrs.get("iteration", 0))
        attrs["physical_proposal_steps"] = saved_steps
        attrs["mcmc_steps_saved"] = saved_steps
        attrs["backend_write_interval"] = 1
        attrs["backend_flush_interval"] = backend_flush_interval
        attrs["stretch_move_a"] = stretch_move_a
        attrs["target_mcmc_proposal_steps"] = target_mcmc_steps

    print(
        f"Recorded {saved_steps} stored MCMC proposal steps in {h5_path}; "
        f"HDF5 writes were flushed every {backend_flush_interval} saved steps."
    )


# Example file path and configuration
filter_wavelength = 670
wavelength_index = 1
...
output_h5_file = "/home/rebeccaz/Github/vampires_calibration/mcmc/results/" + str(filter_wavelength) + "nm_Adjusted_EM_Gain_no_IMR_angle_Angles_PM90_Shared_Priors_NoIMRScipyStart_StretchMove" + stretch_move_label + "_AllSteps_WriteEvery" + str(backend_flush_interval) + "_" + str(target_mcmc_steps) + "_steps.h5"
previous_h5_file = "/home/rebeccaz/Github/vampires_calibration/mcmc/results/" + str(filter_wavelength) + "nm_Adjusted_EM_Gain_no_IMR_angle_Angles_PM90_Shared_Priors_NoIMRScipyStart_1200000_steps.h5"
include_log_f = True
...
# Load p0 dictionary from the matching no-IMR scipy-minimize final fit.
start_txt_file = start_txt_folder + str(filter_wavelength) + "_no_IMR_offset_fixed_EM_gain_no_retardance_constraints_iterate_on_any_improvement_with_dichroic_best_fit_old_fit_values.txt"
...
remaining_mcmc_steps = max(0, target_mcmc_steps - existing_saved_steps)
nsteps = target_mcmc_steps
...
if os.path.exists(output_h5_file):
    with h5py.File(output_h5_file, "r+") as h5_file:
        if "mcmc" in h5_file:
            ...
            attrs["continuation_backend_write_interval"] = 1
            attrs["continuation_backend_flush_interval"] = backend_flush_interval
            attrs["target_mcmc_proposal_steps"] = target_mcmc_steps
            attrs["stretch_move_a"] = stretch_move_a

print(
    f"Targeting {target_mcmc_steps} stored MCMC proposal steps. "
    f"Existing backend has {existing_saved_steps} written samples; "
    f"remaining full-resolution samples to run: {remaining_mcmc_steps}. "
    f"Buffered HDF5 writes will flush every {backend_flush_interval} saved steps."
)
