...
def fit_CHARIS_Mueller_matrix_by_bin_nbs(csv_path, wavelength_bin, new_config_dict_path,plot_path=None):
    ...
    system_dict = {
        "components" : {
            ...
            "nbs" : {
                "type" : "general_retarder_function",
                "properties" : {"phi":  0.5, "delta_theta": 45},
                "tag": "internal",
            },
    ...
    while abs(previous_logl - new_logl) > 0.01*abs(previous_logl):
        ...
        # Configuring minimization function for CHARIS
        result, new_logl, error = minimize_system_mueller_matrix(p0, system_mm, interleaved_values, 
             configuration_list, process_dataset=process_dataset,process_model=process_model,include_sums=False, bounds = [ret_bounds, offset_bounds],mode='least_squares')
        print(result)
